...
def def_admin_change(call):
    ...
    markup.add("گفتگو بین کشور ها🗺️","اختراعات و سناریو📝")
...
@bot.message_handler(func=lambda m: m.text=="ارتباط با ادمین👤" or m.text=="خرید زیرساخت و تجاری🏗️" or m.text=="خرید نظامی 🪖" or m.text=="گفتگو بین کشور ها🗺️" or m.text=="اختراعات و سناریو📝" or m.text=="خرید یورو🛍️"  )
def ability(m):
    cid = m.chat.id
    text=m.text
    ...
    if status=="on":
        ...
        if text=="خرید زیرساخت و تجاری🏗️":
            if start_time <= now <= end_time:
                markup=ReplyKeyboardMarkup(resize_keyboard=True)
                markup.add("لغو و بازگشت به منو")
                bot.send_message(cid,"لطفا اسم زیر ساختی را که میخواهید خریداری کنید ارسال کنید",reply_markup=markup)
                userStep[cid]=1
            else:
            ...
            if start_time <= now <= end_time:
                markup=ReplyKeyboardMarkup(resize_keyboard=True)
                markup.add("لغو و بازگشت به منو")
                bot.send_message(cid,"لطفا اسم وسیله نظامی که میخواهید خریداری کنید را ارسال کنید",reply_markup=markup)
                userStep[cid]=2
            else:
                bot.send_message(cid,"این قابلیت فقط از 4 ظهر تا 9 شب فعال است")
        elif text=="گفتگو بین کشور ها🗺️":
            if start_time <= now <= end_time:   
                markup=ReplyKeyboardMarkup(resize_keyboard=True)
                markup.add("لغو و بازگشت به منو")
                bot.send_message(cid,"لطفا پیام خود را برای گفت و گو با سایر کشور ها ارسال کنید",reply_markup=markup)
                userStep[cid]=3
            else:
                bot.send_message(cid,"این قابلیت فقط از 4 ظهر تا 9 شب فعال است")
        # elif text=="اختراعات و سناریو📝":
